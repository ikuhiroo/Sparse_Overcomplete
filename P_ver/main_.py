...
def main():
    ...
    # 学習済みモデルで初期化
    atom = joblib.load(
        "./trained_model/numIter = 10_l1_reg = 0.5_l2_reg = 1e-5_factor = 3_rate = 0.05_learning rate/newvec_7.pkl"
    )

    Dict = joblib.load(
        "./trained_model/numIter = 10_l1_reg = 0.5_l2_reg = 1e-5_factor = 3_rate = 0.05_learning rate/_dict_7.pkl"
    )
    ...
    Optimizer._del_grad_D = joblib.load(
        "./trained_model/numIter = 10_l1_reg = 0.5_l2_reg = 1e-5_factor = 3_rate = 0.05_learning rate/_del_grad_D7.pkl"
    )
    Optimizer._grad_sum_D = joblib.load(
        "./trained_model/numIter = 10_l1_reg = 0.5_l2_reg = 1e-5_factor = 3_rate = 0.05_learning rate/_grad_sum_D7.pkl"
    )
    Optimizer._del_grad_A = joblib.load(
        "./trained_model/numIter = 10_l1_reg = 0.5_l2_reg = 1e-5_factor = 3_rate = 0.05_learning rate/_del_grad_A7.pkl"
    )
    Optimizer._grad_sum_A = joblib.load(
        "./trained_model/numIter = 10_l1_reg = 0.5_l2_reg = 1e-5_factor = 3_rate = 0.05_learning rate/_grad_sum_A7.pkl"
    )
        ...
        try:
            ...
            for i in range(len(list(var_per_dimention.values()))):
                ...
                if (x.count(0) / len(x)) * 100 < 80:
                    memo_80.append(i)
                elif (x.count(0) / len(x)) * 100 >= 80 and (
                    x.count(0) / len(x)
                ) * 100 < 85:
                    memo_80_85.append(i)
                elif (x.count(0) / len(x)) * 100 >= 85 and (
                    x.count(0) / len(x)
                ) * 100 < 90:
                    memo_85_90.append(i)
                elif (x.count(0) / len(x)) * 100 <= 90 and (
                    x.count(0) / len(x)
                ) * 100 < 95:
                    memo_90_95.append(i)
