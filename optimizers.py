...
class AdaGrad(Optimizer):
    def __init__(self, learning_rate=0.01, eps=1e-6, params=None):
        ...
        self.learning_rate = learning_rate
        self.eps = eps
        self.accugrads = [build_shared_zeros(
            t.shape.eval(), 'accugrad') for t in self.params]
...
class RMSprop(Optimizer):
    def __init__(self, learning_rate=0.001, alpha=0.99, eps=1e-8, params=None):
        ...
        self.mss = [build_shared_zeros(t.shape.eval(), 'ms')
                    for t in self.params]
...
class AdaDelta(Optimizer):
    def __init__(self, rho=0.95, eps=1e-6, params=None):
        ...
        self.accugrads = [build_shared_zeros(
            t.shape.eval(), 'accugrad') for t in self.params]
        self.accudeltas = [build_shared_zeros(
            t.shape.eval(), 'accudelta') for t in self.params]
